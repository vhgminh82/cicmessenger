...
using System.Linq;
using System.Timers;
using CICMessenger.Core.Presence.Transport;
...
namespace CICMessenger.Core.Presence
{
    ...
    {
        public CICMessengerEndPoint User { get; set; } = null!;
    }
    ...
    class KeepAliveService : IDisposable
    {
        ...
        void timer_Elapsed(object? sender, ElapsedEventArgs e)
        {
            ...
            {
                HeIsGone(user);
                UserLost(this, new KeepAliveEventArgs() { User = new CICMessengerEndPoint(user.ID, user.PresenceEndPoint) });
            }
            ...
            foreach (IUserInfo user in going)
                UserLosing(this, new KeepAliveEventArgs() { User = new CICMessengerEndPoint(user.ID, user.PresenceEndPoint) });
        }                        
        ...
        void OnKeepAliveMessage(KeepAliveMessage message)
        {
            ...
            if (existingUser)
                ...
                { 
                    User = new CICMessengerEndPoint(user.ID, user.PresenceEndPoint) 
                });
        }
