...
{    
    class UserDiscovery
    {
        ...
        public void UpdateUser(ICICMessengerEndPoint user, bool discovered)
        {
        ...
        void AskForUserInfo(ICICMessengerEndPoint user, UserInfoState state)
        {
            var reply = Message.FromSender<GiveUserInfoMessage>(thisUser);
            reply.State = (int)state;
            reply.Recipient = new CICMessengerEndPoint(user);
            channel.SendMessage(reply);
        }
