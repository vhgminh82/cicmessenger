...
namespace CICMessenger.Core.Chat.Transport.Host
{
    ...
    class CICMessengerChatGrpcService : CICMessengerChat.CICMessengerChatBase
    {
        ...
        public CICMessengerChatGrpcService(ChatHost chatHost, ILogger logger)
        {
        ...
        public override Task<ChatResponse> SendChatMessage(ChatMessageEnvelope request, ServerCallContext context)
        {
            try
            {
                ...
                var sender = ToCICMessengerEndPoint(request.Sender);
        ...
        void DispatchMessage(Guid sessionId, CICMessengerEndPoint sender, ChatMessageEnvelope envelope)
        {
        ...
        static CICMessengerEndPoint ToCICMessengerEndPoint(EndPointInfo info)
        {
            return new CICMessengerEndPoint(info.ClientId, new IPEndPoint(IPAddress.Parse(info.Ip), info.Port));
        }
    }
}
