...
using CICMessenger.Core.Chat.Transport.Host;
...
namespace CICMessenger.Core.Chat
{
    ...
    {
        ICICMessengerEndPoint localUser;
        ChatHost chatHost;
        Dictionary<string, ICICMessengerEndPoint> remoteUsers;
        ...
        public IEnumerable<ICICMessengerEndPoint> RemoteUsers
        {
            get 
            {
                lock (remoteUsers)
        ...
        public ChatSession(Guid sessionID, ChatHost localHost, ICICMessengerEndPoint localUser, ICICMessengerEndPoint remoteUser, ILogger? logger = null)
        {
            this.Id = sessionID;
            ...
            remoteUsers = new Dictionary<string, ICICMessengerEndPoint>();
            CreateRemoteUsers(Enumerable.Repeat(remoteUser, 1));
        }

        public ICICMessengerEndPoint PrimaryUser
        {
            get 
            { 
                lock (remoteUsers)
        ...
        public void UpdateUser(ICICMessengerEndPoint user)
        {
            AddRemoteUser(user);
        }
        ...
        public void Invite(ICICMessengerEndPoint user)
        {
        ...
        bool IsRemoteUser(ICICMessengerEndPoint user)
        {
            lock (remoteUsers)
                return remoteUsers.ContainsKey(user.ClientID);
        }

        void CreateRemoteUsers(IEnumerable<ICICMessengerEndPoint> users)
        {
            foreach (ICICMessengerEndPoint user in users)
                AddRemoteUser(user);
        }

        void AddRemoteUser(ICICMessengerEndPoint endpoint)
        {
            lock (remoteUsers)
                remoteUsers[endpoint.ClientID] = endpoint;
        }

        void BroadCast(Action<ICICMessengerEndPoint> userAction)
        {
